...
router.post('/gettextsearch', async(req, res) => {
  ...
  res.render('users/scholarshipUserView',{scholarships:saerch_result})
...
router.get('/homepage',isLoggedIn, async (req,res) => {
  ...
  let s_class = await Scholarship.distinct('sclass').exec()
  let scholarshipstype = await Scholarship.distinct('stype').exec()
  let university = await Scholarship.distinct('university').exec()
  ...
  let rec = await Scholarship.find({ stype: auth_email.ustype, sfaculty: { $in: auth_email.ufaculty }, sbranch: { $in: auth_email.ubranch }, sgpa: { $lte: auth_email.ugpa }, sclass: auth_email.uclass }).exec()
  console.log(rec)
  res.render('users/contentpage', { l_costofabode: l_costofabode, l_costoflean: l_costoflean, l_costoflive: l_costoflive, l_country: l_country, university: university, userdata: auth_email, rec: rec, l_facu: l_facu, l_branch: l_branch, s_class: s_class, scholarshipstype: scholarshipstype })
  
  // let l_stype = await Type.distinct('scholarship_type').exec()
  // let l_facu = await Major.distinct('faculty').exec()
  // let l_branch = await Major.distinct('branch').exec()
  // let s_class = await Scholarship.distinct('sclass').exec()
  // let faculty = await Scholarship.distinct('sfaculty').exec()
  // let branch = await Scholarship.distinct('sbranch').exec()
  // let rec = await Scholarship.find({stype:auth_email.ustype,sfaculty:{$in:auth_email.ufaculty},sbranch:{$in:auth_email.ubranch},sgpa:{$lte:auth_email.ugpa},sclass:auth_email.uclass}).exec()
  // console.log(faculty)
  // console.log(branch)
  // res.render('users/contentpage',{userdata:auth_email,rec:rec, faculty: faculty, branch:branch, l_facu: l_facu, l_branch: l_branch,scholarshipstype: l_stype,s_class:s_class })
})
