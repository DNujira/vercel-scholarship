...
router.get('/Maindashboard', async(req, res, next) => {
    ...
    let facultyAmount = await Major.distinct('faculty').count().exec()
    let branchAmonut = await Major.distinct('branch').count().exec()
...
router.get('/list',async (req, res) => {
    ...
    Scholarship.find().limit(150).exec().then((doc, err) => {
        res.render('admins/scholarshiplist', { scholarships: doc , l_facu: l_facu, l_branch: l_branch, s_class: s_class, scholarshipstype: scholarshipstype,l_costofabode: l_costofabode, l_costoflean: l_costoflean, l_costoflive: l_costoflive, l_country: l_country, university: university })
    })
...
router.post('/edit', async(req, res) => {
    ...
    Scholarship.findOne({ _id: edit_id }).exec().then((doc, err) => {
        res.render('admins/editscholarship', { scholarships: doc,scholarshipstype: l_stype, l_facu: l_facu, l_branch: l_branch })
    })
})
